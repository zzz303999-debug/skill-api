...
| 一票多柜 | 同一提单号对应多个柜子，账单中表现为多行 |
| 归集 | 按归集键（业务编号优先、提单号兜底）将多行合并为一个业务订单 |
...
| 成功单注册表 | 提单号 → 首次创建回执的去重登记（imported_orders.json） |
...
- 键规范化：strip 首尾空白 + 统一大写
- 并发：per-bl_no 锁包住「查重→提交→登记」临界区，同提单号跨请求串行化
