...
import app.orders.bill.template_store as template_store
from app.orders.bill.aggregator import _fee_entries
...
class TestFeeEntriesDynamic:
    def test_new_fee_name_auto_collected(self):
        """模板扩展费目（税金）→ 归集自动收录（模拟 _fee_entries 消费侧）。"""
        rows = [BillRow(seq="1", fees={"运费": 2100.0, "税金": 60.0})]
        entries, total = _fee_entries(rows)
        keys = [next(iter(e)) for e in entries]
        assert keys == ["运费", "税金"]  # 首见序（模板声明序）
    ...
    def test_zero_and_text_values_unchanged(self):
        ...
        rows = [
            BillRow(seq="1", fees={"运费": 2100.0, "待时费": 0.0, "其它费": "壹佰元"})
        ]
        entries, total = _fee_entries(rows)
        keys = [next(iter(e)) for e in entries]
    ...
    def test_multi_row_same_fee_accumulates(self):
        """同名费用多行金额累加（行缺键不丢费目）。"""
        rows = [
            BillRow(seq="1", fees={"运费": 100.0}),
            BillRow(seq="2", fees={"运费": 200.0, "税金": 30.0}),
        ]
        entries, total = _fee_entries(rows)
        assert entries[0]["运费"]["money"] == 300.0
        assert entries[1]["税金"]["money"] == 30.0
        assert total == 330.0
...
class TestAiTargetsDynamic:
    def test_schema_enum_tracks_template_additions(self, monkeypatch):
        ...
        import app.orders.bill.ai_header as ai_header
