...
class TestWhitelistLoad:
    async def test_real_config_loaded(self):
    ...
    async def test_missing_config_disables_check(self, tmp_path, monkeypatch):
    ...
    async def test_invalid_config_disables_check(self, tmp_path, monkeypatch):
    ...
    async def test_reload_picks_up_changes(self, tmp_path, monkeypatch):
...
class TestStandardCode:
    ...
    async def test_standard_code_judgement(self, text, expected):
        assert whitelist_module.is_standard_code(text) is expected


class TestCheckUnknown:
    async def test_unknown_standard_code_filtered(self):
        """40GOH（标准形态、不在白名单）→ 拦截。"""
    ...
    async def test_whitelisted_codes_pass(self):
        """白名单内代码（含补充项）→ 放行。"""
        assert whitelist_module.check_unknown_box_types(
            ["40HQ", "20GP", "40OT", "45HQ", "10GP", "53HC"]
        ) == []

    async def test_non_standard_expressions_pass(self):
        """非标表述（既有规则放行）→ 不校验。"""
        assert whitelist_module.check_unknown_box_types(
            ["大冷", "拼箱", "17M飞翼车", "12T", "B/L", "45P1", "2X20"]
        ) == []

    async def test_mixed_keeps_known_removes_unknown(self):
        assert whitelist_module.check_unknown_box_types(
            ["40HQ", "40GOH", "大冷"]
        ) == ["40GOH"]
...
class TestRejectUnknownBoxTypes:
    ...
    async def test_any_unknown_rejects_all_orders(self):
    ...
    async def test_legacy_order_data_box_rejected(self):
    ...
    async def test_multi_unknown_dedup_message(self):
        """多个非法箱型：文件级清单去重保序。"""
        orders = [
...
class TestBuildResultIntegration:
    ...
    async def test_any_unknown_rejects_whole_file_not_submitted(self, monkeypatch):
    ...
    async def test_non_standard_expression_passes(self, monkeypatch):
    ...
    async def test_preview_also_rejects(self, monkeypatch):
