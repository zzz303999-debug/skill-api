"""T7 payload 测试：build_order_payload 快照（对照《逆推规范》§4 字段名）+ b 双写。
...
class TestBuildOrderForm:
    """build_order_payload 字段落点快照（《逆推规范》§4 映射表）。"""

    async def test_required_fields_landing(self):
        form, warnings = build_order_payload(_sample_order())
    ...
    async def test_field_mapping_snapshot(self):
        ...
        form, _ = build_order_payload(_sample_order())
        # 客户：c_title=客户名称（实测响应回显确认）；c_name=客户联系人
    ...
    async def test_multi_container_takes_first_and_warns(self):
        ...
        _, warnings2 = build_order_payload(single)
    ...
    async def test_junyu_customer_fields(self):
        ...
        form, _ = build_order_payload(junyu)
        ...
        form2, _ = build_order_payload(_sample_order(customer_contact="朱经理"))
    ...
    async def test_type_defaults_to_export(self):
        ...
        assert build_order_payload(_sample_order(biz_type="进口"))[0]["type"] == "1"
        assert build_order_payload(_sample_order(biz_type=None, io_type=None))[0]["type"] == "1"

    async def test_payload_flat_form_only(self):
        """TMS 直连表单：扁平字段 + create_order=true；b 双写实测非必需，不再发送。"""
