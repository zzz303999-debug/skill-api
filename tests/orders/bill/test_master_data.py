...
class TestDependencyOrder:
    ...
    async def test_factory_still_blocked_when_all_clients_pending(self, fake_create):
...
class TestPreviewReadOnly:
    ...
    async def test_preview_without_sk_no_pending_note(self, fake_create):
        """owner 化：preview 不带 sk（无归属维度）→ 不标注、不列 pending。"""
        fake_create()
        await run_master_data_async([_make_order()], create_order=True)  # 计数 1（default 槽）
        report = await run_master_data_async([_make_order()], create_order=False)
...
class TestOwnerIsolation:
    ...
    async def test_legacy_flat_file_migrated_to_legacy_slot(self, tmp_path, md_config):
