...
MANIFEST = Path(__file__).parent / "golden" / "tuoshu_cases.json"
PENDING_MANIFEST = Path(__file__).parent / "golden" / "tuoshu_pending_cases.json"
...
def test_mineru_parser_golden_regression(case):
    from app.document_parsers.mineru import parse_pdf
...
def test_image_mineru_parser_regression(case):
    from app.document_parsers.mineru import parse_document
...
def test_quality_router_matches_expected_parser(case, monkeypatch):
    from app.config import settings
    from app.document_parsers.mineru import MinerUParseResult
...
def test_weishi_expected_combines_observed_0615_and_0633_fields():
    observed_dir = Path(__file__).parent / "golden/tuoshu/observed"
    ...
    expected = json.loads(
        (Path(__file__).parent / "golden/tuoshu/expected/A26042709390557851.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_tuoshu_llm_golden_regression(case, monkeypatch):
    ...
    response = skill_module.TuoshuSkill().run(
