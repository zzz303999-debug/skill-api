"""附件文档抽取的纯文本规则族（无 I/O、无编排依赖）。

2026-09 结构整理从 orders/document.py 拆出：归一化（提单号/箱型/件数毛重体积）、
单据修订（关单号/抬头客户/港口/装货时间）、缺失原因判定。编排层（document.py）
import 本模块符号并保持同名 re-export（测试与内部引用不受影响）。
"""
