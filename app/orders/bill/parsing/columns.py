...
从 parser.py 拆出：表头列名匹配（列名/区块.列名/同名#N）、业务列
...
def is_non_fee_header(name: str) -> bool:
    ...
    以「费」结尾豁免（审查 W3）：电话费/手机费/重量费等是真实
...
def discover_fee_columns(
    ...
    # 模板声明的非费目列（fees.ignore_headers）：新式样费用区插入的
