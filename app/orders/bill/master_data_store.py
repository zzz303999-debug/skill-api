...
- 结构：{kind: {key: {count, archive_id, archived_at, last_seen}}}，键 = (kind, 归一键)；
- 语义：跨全部模板家族全局累计（口径 3，用户拍板）；计数与建档登记共用同一记录
  （archive_id 非空 = 已建档，后续直接取用不再建）。
"""
...
class MasterDataStore:
    ...
    def record(self, kind: str, key: str) -> dict[str, Any]:
        """原子自增一次并刷新 last_seen；返回该键最新记录。

        语义：按单计（一单一次，与「出现次数」口径一致，见 T19 同批去重）。
        """
        with self._lock:
            rec = self._data.setdefault(kind, {}).setdefault(key, {})
    ...
    def get(self, kind: str, key: str) -> dict[str, Any] | None:
        """只读当前记录（无记录 → None）。"""
        with self._lock:
            rec = self._data.get(kind, {}).get(key)
            return dict(rec) if rec else None

    def set_archive(
        self, kind: str, key: str, archive_id: str | int
    ) -> dict[str, Any]:
    ...
    def mark_exists_external(self, kind: str, key: str) -> dict[str, Any]:
    ...
    def mark_skip_archive(self, kind: str, key: str) -> dict[str, Any]:
    ...
    def snapshot(self) -> dict[str, dict[str, dict[str, Any]]]:
        """全量快照（报告 TOP 清单/测试断言用）。"""
        with self._lock:
            return {
                kind: {key: dict(rec) for key, rec in entries.items()}
