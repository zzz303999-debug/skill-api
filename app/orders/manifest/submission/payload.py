...
- 类型口径（抓包）：type/ieFlag 恒 2/1（number）；ctnNum number；
...
_TYPE = 2
_IE_FLAG = 1
# 每箱运价默认 0（用户确认）：TMS 误将 unitPrice 设为必填，
...
def build_order_data(order: ManifestOrder) -> dict:
    ...
    return {
        ...
        "cSn": "",
        "cTitle": "",
        # 三栏（名称/地址/电话拆分归位；缺失展示 None、提交转空串）
        ...
        # 门点地址 ← 发货人地址（用户确认：模版有就录入；TMS 三栏地址
        # 与门点地址双写，数据同源）
