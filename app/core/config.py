...
class Settings(BaseSettings):
    ...
    # 关闭思考模式（disabled）大幅降低 reasoning token 与耗时；结构化抽取任务默认关闭；
    # 模型不支持该参数时 client 自动降级重试
    llm_thinking_mode: str = "disabled"

    # Vision
    # LLM 是否具备视觉能力；当前生产模型无视觉 → 图片/扫描件只走 OCR 文本，
    # 禁止把原图发给 LLM
    ...
    # 原图 base64 直传上限（膨胀约 1/3，防超网关请求体限制）；超过跳过 vision 并标记人工复核
    ...
    # 日志与审计留存时长（小时）；审计场景建议调大（720 = 30 天）
