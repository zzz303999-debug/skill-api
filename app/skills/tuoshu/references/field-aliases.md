...
正文抬头/落款中的公司名固定进入 `shipper_agent`。红章、水印和 logo 中的公司名不属于正文，不得抽取。`FROM/FM` 后的联系人进入 `sender_contact`；只有正文明确的发货人/托运人栏位才能进入 `shipper_company`。「公司全称+单据类型」复合抬头（如 `启胜国际物流集团有限公司集卡委托书`）同样取公司全称。
...
`海丰装箱通知` → `海丰`）和正文抬头公司。
「公司全称+单据类型」复合抬头（如 `启胜国际物流集团有限公司集卡委托书`）
属于正文抬头公司来源，取公司全称 `启胜国际物流集团有限公司`；
`集卡委托书`/`委托书`/`托书`/`做箱通知` 等单据类型词尾不进入公司名。
