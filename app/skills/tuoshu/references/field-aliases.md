...
只有原文明示这些标签时才填写。**报关单号不是提单号**，不得放入 `mbl_no`。
...
1. **"关单号"歧义**：**关单号即提单号**，优先当 `mbl_no`；即使与`运编号`/`业务编号`并存也取关单号（运编号归 `internal_ref`）。`报关单号`/`报关号` 归 `customs_declaration_no`，不是提单号。
